...
#     """捕获并记录所有未处理的 Bot 异常，并发送给管理员。"""
#     logger.error("在处理更新时发生异常:", exc_info=context.error)
#
...
#         "更新信息:\n<pre>{}</pre>\n"
#         "错误详情:\n<pre>{}</pre>"
#     ).format(html.escape(str(update)), html.escape(tb_string))
#
...
#             )
#         except Exception:
#             logger.error(f"无法发送错误消息给管理员 {admin_id}。", exc_info=True)
#
...
class TelegramBot:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # 优先检查 proxy_sync 模块是否需要处理此消息
        # proxy_sync 的 MessageHandler 是通过 main.py 的 handle_message 委托调用的
        ...
        if context.user_data.get('searching_nodes'):
            ...
            if keyword: await self.node_handler.handle_search(update, context, keyword)
            else: await update.message.reply_text("⚠️ 请输入有效的搜索关键词")
        elif context.user_data.get('setting_permission'): await self.admin_handler.handle_set_permission(update, context)
        elif context.user_data.get('setting_count'): await self.admin_handler.handle_set_count(update, context)
        elif state == States.AWAITING_ADD: await self.node_handler.handle_add_nodes(update, context)
    ...
    async def _handle_export_selected(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not selected: await query.answer("⚠️ 未选择任何节点", show_alert=True); return
        try:
            import uuid; import yaml
            selected_proxies = [p for p in data_manager.load_proxies() if p.get('name') in selected]
            if not selected_proxies: await query.edit_message_text("⚠️ 未找到选中的节点"); return
            filename = f"selected_nodes_{uuid.uuid4().hex[:8]}.txt"
            with open(filename, "w", encoding='utf-8') as f: yaml.dump(selected_proxies, f, allow_unicode=True)
            with open(filename, "rb") as f: await context.bot.send_document(chat_id=query.message.chat_id, document=f, filename=filename, caption=f"📦 导出的节点文件 ({len(selected_proxies)} 个节点)")
            os.remove(filename)
            await self.node_handler.select_nodes_menu(update, context, 1)
        except Exception as e: await query.edit_message_text(f"❌ 导出失败: {str(e)}")

    async def _handle_clear_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query; await query.answer("🧹 已清空选择")
    ...
    async def _handle_select_all(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query; await query.answer()
        user_id = update.effective_user.id
        if user_id not in self.node_handler.selected_nodes: self.node_handler.selected_nodes[user_id] = set()
        try:
            page = int(query.data.split('_')[-1])
            page_data = data_manager.get_proxies_page(page)
            for proxy in page_data['proxies']: self.node_handler.selected_nodes[user_id].add(proxy.get('name'))
            await self.node_handler.select_nodes_menu(update, context, page)
        except ValueError: await query.answer("❌ 操作失败", show_alert=True)
